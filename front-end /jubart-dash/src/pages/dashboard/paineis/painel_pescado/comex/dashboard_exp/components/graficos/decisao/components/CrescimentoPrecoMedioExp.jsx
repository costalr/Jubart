...
const CrescimentoPrecoMedioExp = ({ exportData, startYear, endYear, startMonth, endMonth }) => {
  ...
  useEffect(() => {
    ...
    const processData = () => {
      ...
      exportData.forEach(({ country, metricFOB, metricKG, year, monthNumber }) => {
      ...
      return Object.entries(priceChanges).map(([country, percentChange]) => ({ country, percentChange }))
    ...
    if (finalData && finalData.length > 0) {
      ...
      setChartData(null); // ensure chart data is null if no data to display
    }
  ...
  return (
    <div>
      ...
      ) : <div>{chartData === null ? "Nenhuma mudança significativa de preço para exibir." : "Carregando..."}</div>}
    </div>
  );
};

export default CrescimentoPrecoMedioExp;
