...
const ToneladasCumulativasExp = ({ startYear, endYear, exportData }) => {
  ...
  useEffect(() => {
    if (!exportData || !Array.isArray(exportData)) {
      console.warn("exportData is not defined or is not an array");
      return;
    }
    ...
    const processData = () => {
      ...
      exportData.forEach(item => {
        ...
        if (!isNaN(year) && !isNaN(month) && year >= startYear && year <= endYear) {
  ...
  }, [startYear, endYear, exportData, showLastFourYears]);
  ...
  const downloadChart = () => {
    if (chartRef.current) {
      ...
      clonedCtx.fillText(
        'Toneladas Cumulativas de Exportação',
      ...
      link.download = 'ToneladasCumulativasExp.png';
      link.href = url;
      link.click();
    }
  };
...
export default ToneladasCumulativasExp;
