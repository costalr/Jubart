...
const ToneladasMensalExp = ({ exportData }) => {
  ...
  useEffect(() => {
    const processData = () => {
      ...
      const filteredData = exportData.filter(d => 
        !isNaN(parseInt(d.year)) && 
        !isNaN(parseInt(d.monthNumber)) &&
        !isNaN(parseFloat(d.metricKG)) &&
        parseInt(d.year) >= 2021 && parseInt(d.year) <= 2024
      );
      ...
      filteredData.forEach(item => {
        ...
        structuredData[year][monthIndex] += parseFloat(item.metricKG) || 0;
      });
...
export default ToneladasMensalExp;
