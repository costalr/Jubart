...
const VariacaoPercentualVolumeAbsoluto = ({ importData }) => {
      ...
      importData.forEach(item => {
        ...
        if (year === 2024) {
          ...
          if (categoryData[category]) {
            const previousVolume = categoryData[category].lastVolume;

            if (previousVolume > 0) {
              const variacaoPercentual = ((volume - previousVolume) / previousVolume) * 100;
              scatterData.push({ x: volume, y: variacaoPercentual, label: category });
            }
          }

          categoryData[category] = { lastVolume: volume };
        }
      });
  ...
  return (
    <div>
      <h2>Análise de Variação Percentual e Volume Absoluto por Categorias para 2024</h2>
